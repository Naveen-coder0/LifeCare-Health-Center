...
import { Calendar, User, Phone, Mail, Clock, CheckCircle2, MapPin, Zap, MessageCircle } from "lucide-react";
...
const AppointmentSection = () => {
  ...
  const validate = () => {
    ...
    if (!form.department) errs.department = "Select a service.";
  ...
  const inputCls = (field: string) =>
    `w-full rounded-xl px-4 py-3.5 text-sm border bg-muted/50 transition-all outline-none focus:bg-white focus:ring-2 focus:shadow-sm ${
      errors[field]
      ...
      <div className="container-clinic mx-auto relative z-10">
        <div className="grid lg:grid-cols-5 gap-12 lg:gap-20 items-start">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Instant confirmation callout */}
            <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-8">
            ...
            <p className="text-muted-foreground leading-relaxed mb-10">
              Fill out the form and our team will confirm your appointment right away.
              Walk-ins are also welcome during working hours.
            </p>

            <div className="space-y-4">
              {[
                { icon: Clock, title: "Working Hours", sub: CLINIC.hours.weekdays },
                { icon: Clock, title: "Sunday", sub: CLINIC.hours.sunday },
                { icon: Phone, title: "Call Us", sub: CLINIC.phoneDisplay },
                { icon: Mail, title: "Email Us", sub: CLINIC.email },
                { icon: MapPin, title: "Location", sub: CLINIC.address },
              ].map((item) => (
                <div key={item.title} className="flex items-center gap-4 group p-3 rounded-2xl hover:bg-muted/50 transition-colors">
                  <div className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 bg-white shadow-sm border border-border/50 group-hover:scale-110 transition-transform duration-300">
            ...
            <a
              ...
              className="mt-8 flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-semibold text-sm bg-[#25D366] text-white hover:bg-[#1ebe5d] transition-all shadow-lg shadow-green-500/20"
            >
              ...
              {submitted ? (
                <div className="text-center py-20 animate-fade-in relative z-10">
                  ...
                  <p className="text-muted-foreground mb-2">
                    ...
                    Our team will call you within minutes to confirm.
                  </p>
                  <p className="text-sm text-muted-foreground mb-10">
                    A confirmation will also be sent to <strong>{form.email}</strong>.
                  </p>
                  <button
                    onClick={() => {
                      setSubmitted(false);
                ...
                <form onSubmit={handleSubmit} noValidate className="relative z-10">
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    ...
                    <div>
                      <select name="doctor" value={form.doctor} onChange={handleChange}
                        className={`${inputCls("doctor")} ${!form.department ? "opacity-50 cursor-not-allowed" : ""}`}
                        disabled={!form.department}>
                        <option value="">Select Doctor *</option>
                        {(doctorsByDept[form.department] || []).map((d) => <option key={d} value={d}>{d}</option>)}
                      </select>
                  ...
                  <button
                    type="submit"
                    className="btn-primary w-full justify-center mt-8 py-4 text-base font-bold shadow-xl shadow-primary/20 hover:shadow-primary/30 transition-all active:scale-[0.98]"
                  >
                    Confirm Appointment — Get Reply in Minutes
                  </button>
